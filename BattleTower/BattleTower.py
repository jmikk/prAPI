...
class BattleTower(commands.Cog):
    ...
    async def _gcog_get_mon_by_uid(self, target: Union[User, Member], uid: str) -> Optional[Dict]:
        """Return the mon dict for a given UID from target user's storage, or None."""
        gcog = self.bot.get_cog("GachaCatchEmAll")
        if not gcog:
            return None
        conf = gcog.config.user(target)
        pokebox = await conf.pokebox()  # list
        team    = await conf.team()     # list

        # Search pokebox (dict entries)
        for m in pokebox or []:
            if isinstance(m, dict) and m.get("uid") == uid:
                return m

        # Search team (dict entries; UID strings are not directly editable here)
        for t in team or []:
    ...
    async def btgrantstats(self, ctx: commands.Context, user: Union[Member, User], uid: str, levels: int):
        """
        Grant *levels* worth of stat points (no level/xp change) to a mon by UID for the specified user.

        Example:
          $btgrantstats @Player 19239d251d01 3
        """
        try:
            await ctx.defer()
        except Exception:
        ...
        # find target mon from the specified user's storage
        mon = await self._gcog_get_mon_by_uid(user, uid)
        if not mon:
            return await ctx.reply(f"Couldn't find a mon with UID `{uid}` for {user.mention}.")

        # require helpers on Gacha cog
        if not hasattr(gcog, "_give_stat_points_for_levels") or not hasattr(gcog, "_auto_allocate_points"):
            return await ctx.reply("Missing helpers on GachaCatchEmAll: _give_stat_points_for_levels / _auto_allocate_points.")
        ...
        # allocate in-place on the mon dict
        try:
            gcog._auto_allocate_points(mon, pts)  # <-- your allocator; 'mon' is the Pokémon dict
        except Exception as e:
            return await ctx.reply(f"Allocation failed: {e!s}")

        mon["bst"] = self._recalc_bst(mon.get("stats", {}))

        # persist to the specified user's storage
        if hasattr(gcog, "_apply_stats_bulk"):
            ...
            # Fallback direct write if bulk helper is absent
