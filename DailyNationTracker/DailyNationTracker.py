...
class DailyNationTracker(commands.Cog):
    ...
    @commands.command()
    async def how_to_DNT(self, ctx):
        """Shows instructions on how to use the Daily Nation Tracker cog."""
        embed = discord.Embed(title="Daily Nation Tracker - How To", color=discord.Color.green())
        embed.add_field(name="Tracking Nations",
                        value="This cog automatically tracks how many days each nation is in the region 'the_wellspring'.",
                        inline=False)
        embed.add_field(name="Setting TG Templates",
                        value="Use `$settemplate <day> %TEMPLATE-XXXXX%` to set a TG template for nations at X days.",
                        inline=False)
        embed.add_field(name="Removing Templates",
                        value="Use `$removetemplate <day>` to remove a template for a specific day.",
                        inline=False)
        embed.add_field(name="Viewing Templates",
                        value="Use `$listtemplates` to view all current day-template settings.",
                        inline=False)
        embed.add_field(name="Reset Data",
                        value="Use `$resetnationdata` to reset all nation day tracking data.",
                        inline=False)
        embed.add_field(name="Import Census Data",
                        value="Use `$importcensusdays` to import days from scale 80 census scores (rounded down).",
                        inline=False)
        embed.add_field(name="View Nation Data",
                        value="Use `$viewnationdata` to view all tracked nations with days, including pagination.",
                        inline=False)
        embed.add_field(name="Send One-Off TG",
                        value="Use `!sendonetg <day> %TEMPLATE-XXXXX% [exact/atleast]` to generate TG links for matching nations.",
                        inline=False)
        embed.set_footer(text="Generated by Daily Nation Tracker")
