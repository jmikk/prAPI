...
class GachaCatchEmAll(commands.Cog):
    ...
    async def exportbox(self, ctx: commands.Context, member: Optional[discord.Member] = None):
        ...
        for e in box_sorted:
            ...
            cards_html.append(f'''
            <div class="card" {data_attrs} data-uid="{uid}">
                ...
                <div class="team">
                    <label class="teamlabel">
                        <input type="checkbox" class="teamchk" data-uid="{uid}" {'disabled' if uid == '?' else ''}>
                        Add to team
                    </label>
                </div>
                <div class="cols">
        ...
        # === Keep the original look/feel; JS/CSS as plain strings to avoid f-string brace issues ===
    ...
    * { box-sizing: border-box; }
    ...
    #q {
      ...
      background: #14161a; color: var(--text); font-size: 1rem; outline: none;
    }
    ...
    /* Team bar + selection highlighting */
    .teambar {
      position: sticky; top: 0; z-index: 5;
      background: rgba(16,17,20,.9); backdrop-filter: blur(6px);
      border: 1px solid #2b2f3a; border-radius: 12px;
      padding: .75rem; margin: .75rem auto 1rem; max-width: 900px;
    }
    ...
    .teamcmd {
      ...
      padding: .55rem .7rem; border-radius: 10px; border: 1px solid #2b2f3a;
      background: #14161a; color: var(--text); font-size: .95rem;
    }
    .copybtn, .clearbtn {
      padding: .5rem .75rem; border-radius: 10px; border: 1px solid #2b2f3a;
      background: #1b1e25; color: #eaeaea; cursor: pointer;
    }
    ...
    """
        ...
        # Search script (unchanged behavior)
        script_search = r"""
    (function() {
      ...
      function matches(card, terms) {
        const hay = (card.dataset.name + ' ' + card.dataset.nick + ' ' + card.dataset.types + ' ' + card.dataset.moves).toLowerCase();
      ...
      function apply() {
        ...
        for (const c of cards) {
          const ok = !terms.length || matches(c, terms);
      ...
      let to = null;
      q.addEventListener('input', () => { clearTimeout(to); to = setTimeout(apply, 80); });
      apply();
    })();
    """
    
        # Team builder script (adds checkboxes -> $team set "UID UID ..." with a 6 cap)
        script_team = r"""
    (function() {
      const MAX = 6;
      const grid = document.getElementById('grid');
      ...
      for (const c of Array.from(grid.querySelectorAll('.card'))) {
        const uid = c.dataset.uid || '';
        if (uid) cardsByUid.set(uid, c);
      }
    
      let team = []; // ordered UIDs
      ...
      function refresh() {
        const atMax = team.length >= MAX;
        for (const chk of chks) {
          if (!chk.checked) chk.disabled = atMax;
        }
        // Build command using string concat (no template literals)
        const cmd = team.length ? ('$team set "' + team.join(' ') + '"') : ('$team set ""');
        teamcmd.value = cmd;
        teamcount.textContent = team.length + '/' + MAX;
      }
    
      function flashLimit(card) {
        if (!card) return;
        card.classList.add('limitshake');
        setTimeout(() => card.classList.remove('limitshake'), 350);
      }
      ...
      function onToggle(e) {
        ...
        if (chk.checked) {
          if (team.length >= MAX) { chk.checked = false; flashLimit(card); return; }
          if (!uid || uid === '?') { chk.checked = false; return; }
          if (!team.includes(uid)) { team.push(uid); setSelected(uid, true); }
        } else {
          team = team.filter(x => x !== uid);
          setSelected(uid, false);
        }
        refresh();
      }
      ...
      function syncFromDOM() {
        ...
        for (const chk of chks) {
          const uid = chk.dataset.uid || '';
          if (chk.checked && uid && uid !== '?') {
            team.push(uid);
            setSelected(uid, true);
          } else {
            setSelected(uid, false);
          }
        }
      ...
      copybtn.addEventListener('click', async () => {
        try {
          await navigator.clipboard.writeText(teamcmd.value);
      ...
      clearbtn.addEventListener('click', () => {
        for (const chk of chks) chk.checked = false;
        syncFromDOM();
      });
    })();
    """
    
        # Final HTML – exact original layout + team bar
      ...
      <div class="container">
        ...
        <div class="teambar" id="teambar">
          ...
          <div class="teamrow">
            ...
            <button id="copybtn" class="copybtn" type="button" title="Copy command">Copy</button>
            <button id="clearbtn" class="clearbtn" type="button" title="Clear team">Clear</button>
          </div>
          <small class="teamhint">Select up to 6 by ticking “Add to team.” The command updates in real time.</small>
        </div>
