...
class GachaCatchEmAll(commands.Cog):
    ...
    s = mon.get("stats", {})
    return int(s.get("hp", 0) + s.get("attack", 0) + s.get("defense", 0)
    ...
    def _pick_damage_move(mon: Dict) -> Optional[Tuple[str, str, str, int]]:
        """Pick a random damage-dealing move (has physical/special + power)."""
        moves = mon.get("moves", [])
        random.shuffle(moves)
        for m in moves:
            name, mtype, style, power = _parse_move(m)
            if style in ("physical", "special") and power and power > 0:
                return (name, mtype, style, power)
        return None

    def _calc_damage(attacker: Dict, defender: Dict, move: Tuple[str, str, str, int]) -> int:
        """Simple damage formula: uses power, (Atk/Def) or (SpA/SpD), STAB, random factor."""
        name, mtype, style, power = move
        astats = attacker.get("stats", {})
        dstats = defender.get("stats", {})

        atk = astats.get("attack", 1)
        dfc = dstats.get("defense", 1)
        if style == "special":
            atk = astats.get("special-attack", 1)
            dfc = dstats.get("special-defense", 1)
        ...
        # STAB
        stab = 1.5 if mtype in attacker.get("types", []) else 1.0
        # Speed tiebreaker slight bonus
        spd_bonus = 1.05 if astats.get("speed", 0) >= dstats.get("speed", 0) else 1.0

        dmg = ((base * (atk / max(1, dfc))) * stab * rand * spd_bonus)
        # Minimal damage
        dmg = max(1, int(dmg))
        return dmg

    def _init_hp(mon: Dict) -> int:
        """Set a reasonable max HP for battle. Use base HP * 5 as a quick, readable scale."""
    ...
    async def battletower(self, ctx: commands.Context, level: int = 100):
        """Battle an endlessly scaling NPC at a chosen level (default 100)."""
        await ctx.defer()

        # Your helpers (must exist on your cog):
        # - _get_user_team(ctx.author) -> List[mon dict]
        # - _generate_npc_team(count, tier) -> List[mon dict]
        # - _tower_scale(mon or list, levels) -> same shape, scaled

        player_team = await self._get_user_team(ctx.author)
        if not player_team:
            return await ctx.reply("You don't have a team set up.")

        player = copy.deepcopy(player_team[0])  # for now: first mon
        player.setdefault("level", 1)

        npc_team = await self._generate_npc_team(1, 1)
        foe = npc_team[0] if isinstance(npc_team, list) else npc_team

        # Scale foe to requested level from its current level
        start_lv = int(foe.get("level", 1))
        lv_diff = max(0, level - start_lv)
        if lv_diff:
            foe = self._tower_scale(foe, lv_diff)

        view = BattleTowerView(ctx, player=player, foe=foe, level_step=5)
        emb = _format_battle_embed("Team Battle — Battle Tower",
                                player, _init_hp(player), _init_hp(player),
                                foe, _init_hp(foe), _init_hp(foe),
                                footer="Choose a move or ⏩ Auto-Sim to results.")
...
class BattleTowerView(discord.ui.View):
    def __init__(self, ctx: commands.Context, player: Dict, foe: Dict, level_step: int = 5, timeout: int = 120):
        super().__init__(timeout=timeout)
        self.ctx = ctx
        self.user_id = ctx.author.id
        self.level_step = level_step

        # Deep copies to keep originals safe
        ...
        # Prepare move buttons for player (up to 4)
    ...
    def _add_move_buttons(self):
        moves = self.player.get("moves", [])[:4]
        # Filter to damage-dealing first; if none, fall back to first 4 moves anyway
        dmg_moves = []
        for m in moves:
            name, mtype, style, power = _parse_move(m)
            if style in ("physical", "special") and power and power > 0:
        ...
        for idx, m in enumerate(moves):
            name, mtype, style, power = _parse_move(m)
            label = f"{name} ({power})"
            self.add_item(self.MoveButton(self, label=label, move_tuple=(name, mtype, style, power), row=0 if idx < 3 else 1))

    # ---------- Turn Resolution ----------
    async def do_turn(self, interaction: discord.Interaction, move: Tuple[str, str, str, int], autosim: bool = False):
        # Block others
        ...
        # Check victory
        if self.f_cur <= 0:
            await self._on_victory(interaction, move_name=move[0], dealt=p_dmg)
            return

        # Foe counters
        foe_move = _pick_damage_move(self.foe)
        if not foe_move:
            # If foe somehow has no damage move, use a tiny fixed poke
            foe_move = ("struggle", "normal", "physical", 40)
        f_dmg = _calc_damage(self.foe, self.player, foe_move)
        self.p_cur = max(0, self.p_cur - f_dmg)

        # Check defeat
        if self.p_cur <= 0:
            await self._on_defeat(interaction, foe_move_name=foe_move[0], dealt=f_dmg)
            return

        # Update embed
        footer = f"You used {move[0]} (dealt {p_dmg}). Foe used {foe_move[0]} (dealt {f_dmg})."
        emb = _format_battle_embed("Team Battle — Battle Tower", self.player, self.p_cur, self.p_max, self.foe, self.f_cur, self.f_max, footer=footer)
        await interaction.response.edit_message(embed=emb, view=self)

        # If autosim, schedule another quick turn with a random player move
        if autosim:
            await interaction.followup.send(content="⏩ Auto-sim continues…", ephemeral=True)
            next_move = _pick_damage_move(self.player)
            if next_move:
                # Fake an interaction by reusing the last message
                msg = await interaction.original_response()
                fake = object()  # sentinel
                await self.do_turn(interaction, next_move, autosim=True)

    async def _on_victory(self, interaction: discord.Interaction, move_name: str, dealt: int):
        # Compute EXP by BST difference (clamped) + level factor
        p_bst = _bst(self.player)
        f_bst = _bst(self.foe)
        level = int(self.foe.get("level", 1))
        diff = f_bst - p_bst
        exp = max(10, int(diff / 4) + level * 2)

        # Disable current buttons, replace with rematch buttons
        self.clear_items()
        self.add_item(self.RematchSameButton(self))
        self.add_item(self.RematchHigherButton(self))
        self.add_item(self.CloseButton())

        footer = f"✅ Victory! Your {self.player['name'].title()} used {move_name} for {dealt} damage.\nYou gained **{exp} EXP**."
        emb = _format_battle_embed("Victory — Battle Tower", self.player, self.p_cur, self.p_max, self.foe, self.f_cur, self.f_max, footer=footer)
        await interaction.response.edit_message(embed=emb, view=self)

        # You might want to actually apply EXP to your user's mon here (call your own exp/level system).

    async def _on_defeat(self, interaction: discord.Interaction, foe_move_name: str, dealt: int):
        self.clear_items()
        self.add_item(self.CloseButton())

        footer = f"💀 Defeat. Foe used {foe_move_name} for {dealt} damage."
        emb = _format_battle_embed("Defeat — Battle Tower", self.player, self.p_cur, self.p_max, self.foe, self.f_cur, self.f_max, footer=footer)
        await interaction.response.edit_message(embed=emb, view=self)

    # ---------- Buttons ----------
    class MoveButton(discord.ui.Button):
        def __init__(self, parent: "BattleTowerView", label: str, move_tuple: Tuple[str, str, str, int], row: int = 0):
            super().__init__(style=discord.ButtonStyle.primary, label=label, row=row)
            self.parent = parent
            self.move_tuple = move_tuple

        async def callback(self, interaction: discord.Interaction):
            await self.parent.do_turn(interaction, self.move_tuple, autosim=False)
    ...
    class AutoSimButton(discord.ui.Button):
        ...
        async def callback(self, interaction: discord.Interaction):
            mv = _pick_damage_move(self.parent.player)
            if not mv:
                return await interaction.response.send_message("No damaging move to Auto-Sim with.", ephemeral=True)
            await self.parent.do_turn(interaction, mv, autosim=True)
    ...
    class GiveUpButton(discord.ui.Button):
        ...
        async def callback(self, interaction: discord.Interaction):
            ...
            self.parent.add_item(BattleTowerView.CloseButton())
            emb = _format_battle_embed("You Gave Up — Battle Tower",
    ...
    class RematchSameButton(discord.ui.Button):
        def __init__(self, parent: "BattleTowerView"):
        ...
        async def callback(self, interaction: discord.Interaction):
            if interaction.user.id != self.parent.user_id:
                return await interaction.response.send_message("This isn't your battle.", ephemeral=True)
            # Reset foe at same level
            foe_template = copy.deepcopy(self.parent.foe)
            # Recompute max/current HP
            self.parent.f_cur = self.parent.f_max = _init_hp(foe_template)
            # Refill buttons
            self.parent.clear_items()
            self.parent._add_move_buttons()
            self.parent.add_item(BattleTowerView.AutoSimButton(self.parent))
            self.parent.add_item(BattleTowerView.GiveUpButton(self.parent))
    ...
    class RematchHigherButton(discord.ui.Button):
        def __init__(self, parent: "BattleTowerView"):
        ...
        async def callback(self, interaction: discord.Interaction):
            if interaction.user.id != self.parent.user_id:
                return await interaction.response.send_message("This isn't your battle.", ephemeral=True)
            # Scale foe upward by step
            step = self.parent.level_step
            # If you have a helper on the Cog to scale: call it via the cog method on ctx.cog
            cog = interaction.client.get_cog(self.parent.ctx.cog.__class__.__name__)
            if cog and hasattr(cog, "_tower_scale"):
                self.parent.foe = cog._tower_scale(self.parent.foe, step)
            else:
                # Fallback: bump stats a bit if scaling helper unavailable
                s = self.parent.foe.get("stats", {})
                for _ in range(step):
                    for k in s:
                        s[k] += random.choice([0,1,1,2,2,3])
                self.parent.foe["stats"] = s
                self.parent.foe["level"] = self.parent.foe.get("level", 1) + step
