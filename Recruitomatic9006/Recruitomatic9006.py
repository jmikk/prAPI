    ...
    async def callback(self, interaction):
        try:
            ...
            if interaction.user.id == self.invoker_id:
                ...
                await Recruitomatic9006.send_nations_file(Recruitomatic9006, self.ctx)
...
class Recruitomatic9006(commands.Cog):
    ...
    async def send_nations_file(self, ctx):
        ...
        with open(filename, "rb") as file:
            await ctx.send(f"Here's the list of all nations that earned tokens, you earned: {len(nations_tged)}", file=discord.File(file, filename))
